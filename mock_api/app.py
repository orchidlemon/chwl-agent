...
_SCENARIO_ACCEPTED = {
    "couple":  ["couple",  "both", "friends"],
    "solo":    ["solo",    "both", "friends"],
    "family":  ["family",  "both"],
    "friends": ["friends", "both"],
}
...
def item_matches_preferences(item, preferences):
    ...
    # Also match by cuisine (e.g. "川菜") and tags
    if item.get("cuisine"):
        searchable.add(item["cuisine"])
    searchable.update(item.get("tags", []))
...
class MockApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/route/estimate":
            ...
            base    = ROUTES.get((from_id, to_id, "taxi")) or ROUTES.get((to_id, from_id, "taxi"))
            if not base:
                base = {"distance_km": 3.2, "duration_min": 16,
                        "walk_distance_m": 120, "traffic_level": "normal"}
            dist = base.get("distance_km", 3.2)
            base_dur = base.get("duration_min", 16)
            mode_dur = {
                "taxi":    base_dur,
                "driving": max(5, int(base_dur * 0.75)),
                "transit": max(8, int(base_dur * 1.3)),
                "walking": max(10, int(dist * 12)),   # ~5 km/h
            }.get(mode, base_dur)
            route = {
                "from": from_id, "to": to_id, "mode": mode,
                "distance_km": dist,
                "duration_min": mode_dur,
                "walk_distance_m": int(dist * 1000) if mode == "walking" else base.get("walk_distance_m", 120),
                "traffic_level": base.get("traffic_level", "normal"),
                "source": "mock_computed",
            }
    ...
    def do_POST(self):
        ...
        if path == "/api/taxi/dispatch":
            import random
            pool = STATE.get("taxi_pool", [])
            if not pool:
                json_response(self, 503, {"error": "no_taxis_available"})
                return
            taxi = copy.deepcopy(random.choice(pool))
            taxi["eta_min"] = random.randint(2, 6)
            taxi["estimated_fare"] = random.randint(12, 48)
            taxi["status"] = "dispatched"
            json_response(self, 200, taxi)
            return
